...
class TerminalPin:
    ...
    # PCB component designator that owns this pad (e.g. ``J1``). Set when the
    # pin is built from a :class:`RawPad`; useful when a terminal aggregates
    # pads from several components via ``PDN_*_DES``.
...
def _resolve_two_terminal(
    ...
    p_spec, p_err = _side(p_net, p_pins, p_des, "P")
    n_spec, n_err = _side(n_net, n_pins, n_des, "N")
